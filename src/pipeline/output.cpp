...
class string_buffer {
 public:
  hash add(const std::string& str) {
    auto hash = hans::engine::hasher(str.c_str());
...
struct resource_info {
  engine::Configurator::Resources type;
  size_t value;
};
...
class Configurator : public hans::engine::Configurator {
 public:
...
using task_fn = std::function<bool(const user_data&, EngineData&, context&)>;
...
static user_object find_object(const user_data& input, hash variable,
  ...
  for (const auto& object : objects) {
    auto _variable = hans::engine::hasher(object.name.c_str());
    auto _program = hans::engine::hasher(object.program.c_str());
...
static bool configure_graph(hans::engine::PluginManager& plugins,
                            hans::Graphs& graphs, const user_data& input,
                            EngineData& output, context& ctx) {
...
static bool configure_task(const user_data& input, EngineData& output,
                           context& ctx) {
  hans::engine::StringManager strings(ctx.strings.primitive());
  hans::engine::PluginManager plugins(strings, output.plugins);
...
static bool registers_allocate(const std::vector<user_connection>& connections,
  ...
  for (const auto& connection : connections) {
    auto source = hans::engine::hasher(connection.source.c_str());
    auto sink = hans::engine::hasher(connection.target.c_str());
    auto found = 0;
...
static bool shaders_task(const user_data& input, EngineData& output,
  ...
  for (const auto& object : input.objects) {
    auto type = hans::engine::hasher(object.type.c_str());
...
static bool modulators_task(const user_data& input, EngineData& output,
                            context& ctx) {
  for (const auto& program : input.programs) {
    auto pgm = hans::engine::hasher(program.name.c_str());
...
bool hans::pipeline::output(const user_data& input, EngineData& output) {
  context ctx(input);
