...
class GuilePatcher : public Configurator {
 public:
  ...
  virtual void request(Configurator::Resources type, size_t value) {
    auto& smobs = scm::detail::Smobs::get();
    auto scm_type = smobs.enum_to_scm("RESOURCES", type);

    switch (type) {
    case Configurator::Resources::INLET:
    case Configurator::Resources::OUTLET: {
      m_requests.push_back(scm_cons(scm_type, scm_from_int(value)));
      break;
    }
    case Configurator::Resources::RING_BUFFER: {
